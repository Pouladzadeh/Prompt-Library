...
<output>Slide Number → Title → Purpose → Transition Note → Cut/Merge Flags</output>
...
- Insight must be genuinely non-obious — not common knowledge
- One takeaway only — not three, not five, one
</rules>

<output>Hook → Problem → Insight → Solution → Single Takeaway</output>
...
<output>Per Slide: Layout → Visual Type → Chart/Diagram Recommendation → Design Language Summary</output>
...
<output>Per Slide: 3-5 Bullet Points → Clarity Check → Overloaded Slide Flags</output>
...
<output>Flagged Slides → Rewritten Content → Before/After per Slide → Final Slide Count</output>
